...
class PostAdapterImport() : RecyclerView.Adapter<PostAdapterImport.MyViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        return MyViewHolder(
            LayoutInflater.from(context).inflate(
                R.layout.items_import_recycler,
                parent,
                false
            )
        )
        ...
        when {
            posts[pos].type == "increase" -> {
                holder.description.text =
                    posts[pos].name.toString() + " has increased price of " + posts[pos].product + " by USD " + posts[pos].change + " to USD " + posts[pos].newPrice + " per kg(FOB)"
                holder.image.setImageResource(R.drawable.ic_arrow_upward)
            }
            posts[pos].type == "decrease" -> {
                holder.description.text =
                    posts[pos].name.toString() + " has reduced price of " + posts[pos].product + " by USD " + posts[pos].change + " to USD " + posts[pos].newPrice + " per kg(FOB)"
                holder.image.setImageResource(R.drawable.ic_arrow_downward)
            }
            else -> holder.description.text =
                posts[pos].name.toString() + " has added a new product " + posts[pos].product + " at USD" + posts[pos].newPrice + " per kg(FOB)"
        }
